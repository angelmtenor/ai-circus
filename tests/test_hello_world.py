...
# ai_circus.tools.__init__ re-exports main as the name "hello_world", which shadows
# the submodule of the same name — import it directly to get the module.
hello_world = importlib.import_module("ai_circus.tools.hello_world")
...
def test_main_configures_logging_and_greets_with_llm_provider(monkeypatch: pytest.MonkeyPatch) -> None:
    """main() should configure logging, log system info, and greet using the configured LLM_PROVIDER."""
    calls: list[str] = []
    ...
    assert calls == ["configure_logger", "info_system", "Hello, google!"]
